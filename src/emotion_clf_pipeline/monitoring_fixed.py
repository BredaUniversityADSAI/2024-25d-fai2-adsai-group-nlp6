...
class MetricsCollector:
        ...
        try:
            ...
            for feature, values in feature_means.items():
                if feature in baseline_means:
                    ...
                    if baseline_mean != 0:
                        drift_score = abs(recent_mean - baseline_mean) / abs(baseline_mean)
    ...
    def _update_api_metrics(self):
        ...
        try:
            ...
            if latencies:
                p50_latency = np.percentile(latencies[-100:], 50)  # Last 100 predictions
    ...
    def generate_daily_summary(self):
        ...
        try:
            summary = {
                ...
                "total_predictions": self.total_predictions,
                "total_errors": self.total_errors,
                "error_rate_percent": (self.total_errors / max(self.total_predictions, 1)) * 100,
                "uptime_hours": (datetime.now() - self.start_time).total_seconds() / 3600,
                "avg_latency": np.mean(self.histograms.get("prediction_latency", [0])),
                "p95_latency": np.percentile(self.histograms.get("prediction_latency", [0]), 95),
        ...
        try:
            ...
            summary = {
                ...
                "api_metrics": {
                    ...
                    "total_errors": self.total_errors,
                    "active_requests": self.active_requests,
                    "prediction_rate_per_minute": (self.total_predictions / max(uptime, 1)) * 60,
                    "error_rate_percent": (self.total_errors / max(self.total_predictions, 1)) * 100,
                },
                "latency_metrics": {
                    "avg": np.mean(self.histograms.get("prediction_latency", [0])),
                    "p50": np.percentile(self.histograms.get("prediction_latency", [0]), 50),
                    "p95": np.percentile(self.histograms.get("prediction_latency", [0]), 95),
                    "p99": np.percentile(self.histograms.get("prediction_latency", [0]), 99),
                },
                "model_performance": self.model_performance.copy(),
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics_collector.histograms[f"{operation_name}_latency"].append(latency)
