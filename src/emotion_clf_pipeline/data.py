...
class FeatureExtractor:
    ...
    def extract_all_features(self, text, expected_dim=None):
        ...
        Args:
            ...
            expected_dim (int, optional): The dimension the output features should have.
                                       If None, uses the natural dimension of enabled features.
            
        Returns:
            numpy.ndarray: Combined array of all features, padded/truncated to expected_dim if provided.
        """
        features_list = []
        ...
        if self.feature_config.get('tfidf', True):
            # Ensure TF-IDF vectorizer is fitted, especially if called outside DataPreparation context
            if self.tfidf_vectorizer is None:
                # Fallback: return zeros for TF-IDF part if not fitted, or fit with dummy if appropriate
                # For prediction, it should have been fit during setup of CustomPredictor.
                # However, if CustomPredictor.predict fits it on the fly, that's different.
                # Let's assume for now if it's None, it means 0 features for this part for a single text.
                num_tfidf_features = 100 # Default expected, should align with get_feature_dim
                if hasattr(self, '_actual_tfidf_dim') and self._actual_tfidf_dim is not None:
    ...
    def get_feature_dim(self, expected_dim_from_model=None):
        ...
        If expected_dim_from_model is provided, it might influence calculations for uninitialized parts (like TF-IDF).
        However, this function should primarily reflect the natural dimension based on config.
        The padding/truncation should ideally happen in extract_all_features.
        
        Returns:
            int: Total dimension of all enabled features based on current configuration.
        """
        total_dim = 0
        if self.feature_config.get('pos', True): total_dim += 10
        if self.feature_config.get('textblob', True): total_dim += 2
        if self.feature_config.get('vader', True): total_dim += 4
        if self.feature_config.get('emolex', True):
            if self.emolex_lexicon is not None: # Only add if lexicon loaded
                 total_dim += (8 * 2) + 2 + 3 # 21 total EmoLex features
            # else: if emolex is True but lexicon is None, it will produce zeros but occupy space if expected.

        if self.feature_config.get('tfidf', True):
            if self.tfidf_vectorizer is not None and hasattr(self.tfidf_vectorizer, 'max_features'):
                # Use max_features if vectorizer is initialized, as this is the intended fixed dimension
                self._actual_tfidf_dim = self.tfidf_vectorizer.max_features
                total_dim += self._actual_tfidf_dim
            elif self.tfidf_vectorizer is not None and hasattr(self.tfidf_vectorizer, 'get_feature_names_out'):
                # Fallback if max_features isn't directly on the instance but vocab is there
                # This case should ideally be avoided by consistent TfidfVectorizer setup
                self._actual_tfidf_dim = len(self.tfidf_vectorizer.get_feature_names_out())
                total_dim += self._actual_tfidf_dim
            else:
                # If TF-IDF is enabled in config but vectorizer not set or lacks max_features info, assume default.
                # This should align with TfidfVectorizer(max_features=100) default in fit_tfidf
...
class DataPreparation:
    ...
    def __init__(self, output_columns, tokenizer, max_length=128, batch_size=16, feature_config=None):
        ...
        _project_root_dir_dp = os.path.dirname(os.path.dirname(os.path.dirname(_current_file_path_dp)))
        emolex_lexicon_path = os.path.join(_project_root_dir_dp, "models", "features", "EmoLex", "NRC-Emotion-Lexicon-Wordlevel-v0.92.txt")

        # Initialize feature extractor with configuration and lexicon path
        self.feature_extractor = FeatureExtractor(feature_config=feature_config, lexicon_path=emolex_lexicon_path)
        
        # Define output columns that will be used for labels
        # self.output_columns = ['emotion', 'sub_emotion', 'intensity']
        
        # The following individual extractors are redundant as FeatureExtractor handles them
        # self.pos_extractor = POSFeatureExtractor()
        # self.textblob_extractor = TextBlobFeatureExtractor()
        # self.vader_extractor = VaderFeatureExtractor()
        # self.emolex_extractor = EmolexFeatureExtractor(lexicon_path=emolex_lexicon_path) # Corrected if it were needed
...
class EmotionDataset(Dataset):
    ...
    def __init__(self, texts, tokenizer, features, labels=None, feature_extractor=None, max_length=128, output_tasks=None):
        """
        ...
        Args:
            ...
            labels (list, optional): List of label tuples (emotion, sub_emotion, intensity). None for prediction.
            feature_extractor (FeatureExtractor, optional): Feature extractor instance. Not strictly needed if features are pre-computed.
            max_length (int): Maximum sequence length for BERT
            output_tasks (list, optional): List of tasks to output. Used only if labels are provided.
        """
        self.texts = texts
        self.tokenizer = tokenizer
        self.features = features # Should be pre-calculated and correctly dimensioned
        self.labels = labels
        # self.feature_extractor = feature_extractor # Not used if features are pre-supplied
        self.max_length = max_length
