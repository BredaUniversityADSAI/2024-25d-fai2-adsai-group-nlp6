...
def speech_to_text(transcription_method, audio_file, output_file):
    ...
    try:
        if transcription_method.lower() == "assemblyai":
            ...
            if not api_key:
                raise ValueError("AssemblyAI API key not found in environment variables (checked in function)")
...
def process_youtube_url_and_predict(youtube_url: str, output_filename_base: str, transcription_method: str) -> list[dict]:
    """
    ...
    Returns:
        list[dict]: A list of prediction dictionaries, one for each transcribed sentence.
                     Each dictionary contains 'emotion', 'sub_emotion', 'intensity'.
    """
    print(f"Starting emotion prediction pipeline for URL: {youtube_url}")
    print(f"Using transcription method: {transcription_method}, Filename base: {output_filename_base}")
    ...
    # Assuming save_youtube_audio is available (it is in data.py, ensure it's imported in predict.py if not already)
    # from data import save_youtube_audio # This import might be needed at the top of predict.py
    ...
    transcript_output_file = os.path.join(transcripts_dir, f"transcribed_data_{output_filename_base}_{transcription_method}.xlsx")
    speech_to_text(
    ...
    if not os.path.exists(transcript_output_file):
        ...
        # e.g., by capturing stdout/stderr from speech_to_text or checking API key presence
        # For now, a generic message:
        error_message = f"Transcription failed: Output file {transcript_output_file} not found. "
        if transcription_method.lower() == "assemblyai" and not os.environ.get("ASSEMBLYAI_API_KEY"):
            error_message += "AssemblyAI API key is missing. Please set the ASSEMBLYAI_API_KEY environment variable."
        else:
    ...
    if sentences: # Only call predict_emotion if there are sentences
        # predict_emotion is expected to return a list of dicts, or None if an error occurs
    ...
    if raw_predictions is None:
        print("Warning: Emotion prediction step returned None. Emotion data will be marked as N/A.")
    ...
    if sentences and num_rows_in_df > 0:
        if raw_predictions and len(raw_predictions) == num_rows_in_df:
            # Predictions exist and their count matches the number of rows in the DataFrame
            for i, pred_dict in enumerate(raw_predictions):
                if isinstance(pred_dict, dict):
                    ...
                    # Ensure intensity is stored as a string, consistent with API response model
                    intensities_column_data[i] = str(pred_dict.get("intensity", pd.NA))
                # else: data for this row remains pd.NA as initialized
            print("Emotions, sub-emotions, and intensity data processed from predictions.")
        elif raw_predictions: # Predictions exist but length mismatches
            print(f"Warning: Mismatch between number of predictions ({len(raw_predictions)}) and DataFrame rows ({num_rows_in_df}). Emotion data will be N/A.")
            # Data columns remain as pd.NA (already initialized)
        else: # No predictions returned (raw_predictions is empty), but there were sentences
            print("Warning: Emotion prediction yielded no results for the provided sentences. Emotion data will be N/A.")
            # Data columns remain as pd.NA (already initialized)
    elif num_rows_in_df == 0:
         print("Info: Transcript DataFrame is empty. Emotion columns will be empty.")
         # emotions_column_data etc. are already [], which is correct for an empty DataFrame.
    # else: No sentences to begin with, num_rows_in_df might be >0 if they were all NaN and dropped.
    # Handled by initialization if num_rows_in_df > 0, or if num_rows_in_df == 0.
    ...
    results_output_file = os.path.join(results_dir, f"results_{output_filename_base}_{transcription_method}.xlsx")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Emotion Classification Pipeline for YouTube URLs")
    ...
    parser.add_argument('--filename', type=str, 
                      ...
                      help='Base filename for the downloaded audio and other outputs (without extension)')
